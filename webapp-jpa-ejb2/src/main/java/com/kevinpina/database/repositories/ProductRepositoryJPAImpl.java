...
//		return entityManager.createQuery("FROM Product", Product.class).getResultList();
		...
		/*
		* If we change above for return entityManager.createQuery("FROM Product", Product.class).getResultList(); that
		* now JPA handle the EntityManager and everytime fetch a Product will close automatically the connection
		* when try to get a category which is Lazy will throw an Exception due to connection is already close.
		* 		@ManyToOne(fetch = FetchType.LAZY)
		* 		private Category category;
		* so always is a good practice use left outer join fetch to our relations.
		* */
	}
